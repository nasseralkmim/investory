...
hledger -f data/index.ledger -f data/^spx.ledger \
        ...
        --begin=2023-01-01 --end=2024-01-01 > data/spx-twr.org
hledger -f all.ledger -f data/aapl.ledger \
        roi --inv investments --pnl unrealized \
        --value=then --monthly --infer-market-price \
        --begin=2023-01-01 --end=2024-01-01 > data/portfolio-twr.org
#+end_src
...
#+begin_src sh :dir examples/reports :results output :async yes :session inv-perf
python -m investory.values --commodity aapl --output-dir data --begin 2023-01-01
# python -m investory.values --commodity € --yahooticker EURUSD=X --output-dir data --begin 2023-01-01
python -m investory.reports --ledger all.ledger --data-dir data --verbose 0 --currency $
#+end_src

#+RESULTS:
#+begin_example
((general) ) 𒆸﻿ ((general) ) 𒆸﻿ Output directory: /home/nasser/.local/src/investory/examples/reports/data
Ledger file path: /home/nasser/.local/src/investory/examples/reports/data/aapl.ledger
Last date recorded in /home/nasser/.local/src/investory/examples/reports/data/aapl.ledger: 2025-03-31 00:00:00
Fetched history_data shape: (18, 5)
Target month-end dates range: NaT to NaT
Relevant data shape before filtering processed dates: (0, 5)
/home/nasser/.local/src/investory/investory/values.py:239: FutureWarning: The behavior of 'isin' with dtype=datetime64[ns] and castable values (e.g. strings) is deprecated. In a future version, these will not be considered matching by isin. Explicitly cast to the appropriate dtype before calling isin instead.
  relevant_data = relevant_data[~relevant_data.index.map(lambda d: d.date()).isin(processed_dates)]
Relevant data shape AFTER filtering processed dates: (0, 5)
No new month-end data to write.
((general) ) 𒆸﻿ ((general) ) 𒆸﻿ ((general) ) 𒆸﻿
#+end_example
